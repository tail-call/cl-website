...
(defun run-server (port)
  ...
  (let ((socket (make-socket '(127 0 0 1) port)))
    (unwind-protect
        (loop
            (accept-one-stream socket (lambda (request)
